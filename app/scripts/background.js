	...
	try {
		chrome.tabs.query({url: "https://*.force.com/*"}, tabs => {
			for(let i = 0; i < tabs.length; i++)
				chrome.tabs.sendMessage(tabs[i].id, message, response => log(tabs[i].url))
		})
...
log("sess have")
		promiseGetSessionCommands(args)
		sendResponse({
...
// ------------- Promises
var promiseGetSessionCommands = args => {
log("promise commands")
	let sessionHash, url
...
log("promise retrieve commands")
		commands[sessionHash] = { "Refresh Metadata": {}, "Toggle Lightning": {}, "Setup": {}, "?": {}, "Home": {url: "/"} }
		if(serverInstances[sessionHash].includes("lightning.force")) {
			commands[sessionHash]["Setup"] = {url: "/lightning/setup/SetupOneHome/home"}
			commands[sessionHash]["Toggle Lightning"] = {url: "/ltng/switcher?destination=classic"}
		}
		else {
			commands[sessionHash]["Setup"] = {url: "/ui/setup/Setup"}
			commands[sessionHash]["Toggle Lightning"] = {url: "/ltng/switcher?destination=lex-campaign"}
		}

		promiseGetSetupTree(sessionHash).then(promiseGetMetadata.bind(null,sessionHash)).then(promiseGetCustomObjects.bind(null,sessionHash)).then(()=>{
			sendTabMessage({action: 'updateSessionCommands', key: sessionHash, commands: commands[sessionHash] })
		})
	}
	return args
}
var promiseGetSetupTree = sessionHash => {
	return new Promise(resolve => {
		resolve( promiseHttp({targetUrl: apiUrls[sessionHash] + "/ui/setup/Setup", type: "document"}) )
	}).then(response => {
log("promise parse setup tree")
...
var promiseGetCustomObjects = sessionHash => {
	return new Promise(resolve => {
		resolve( promiseHttp({ targetUrl: apiUrls[sessionHash] + "/p/setup/custent/CustomObjectsPage", type: "document" }) )
	})
	.then(response => {
log("promise parse custom")
...
var promiseGetMetadata = sessionHash => {
	return new Promise(resolve => {
		resolve( promiseHttp({
			targetUrl: apiUrls[sessionHash] + '/services/data/' + SFAPI_VERSION + '/sobjects/',
			headers: {
				"Authorization": "Bearer " + sessionIds[sessionHash],
				"Accept": "application/json"
			}}
		))
	})
	.then(response => {
log("promise parse meta")
